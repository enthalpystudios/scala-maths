...
import net.projecteuler.largestproduct.{CoordinatesProductCalculator, GridCompassExtractor, LargestProductInGridFinder}
...
*/
class LargestProductInGridSpec extends FeatureSpec with GivenWhenThen with Matchers {

  scenario("Return the largest product of a grid") {
  ...
  scenario("extract n numbers in a specific direction") {

    Given("a grid of 20x20 and a position")
    val gridCoordMap: Map[(Int, Int), Int] = createGrid
    val position = (0, 0)

    When("Call the extractor to get the available coordinates for the given position")
    val coordinates: Seq[Seq[(Int, Int)]] = GridCompassExtractor.extract(0 to 3)(position)

    val expectedEastCoords = Seq(
        (0,0), (0,1), (0,2), (0,3)
    )
    val expectedSouthEastCoords = Seq(
        (0,0), (1,1), (2,2), (3,3)
    )
    val expectedSouthCoords = Seq(
        (0,0), (1,0), (2,0), (3,0)
    )

    Then("The result should be the east, south and southeast coordinates")
    coordinates.size should be(8)
    coordinates should contain(expectedEastCoords)
    coordinates should contain(expectedSouthEastCoords)
    coordinates should contain(expectedSouthCoords)

  }

  scenario("calculate the product of a given coordinate longitude") {

    Given("a grid of 20x20 and a coordinate of longitude 4 starting at (0,0)")
    ...
    When("The calculator is called with some coordinates")
    val productCalculatorForCoordinates = CoordinatesProductCalculator.calculate(gridCoordMap)(coordinates)

    val expectedMultiplicationOfCoordinates = List(34144, 279496, 1651104)

    Then("The multiplication of the values of those coordinates should be the expected")
    productCalculatorForCoordinates should contain theSameElementsAs(List(34144, 279496, 1651104))

  }

  private def createGrid: Map[(Int, Int), Int] = {
    val grid = Array(
    ...
    val gridCoordMap = (for {
      i <- (0 to 19)
      j <- (0 to 19)
    } yield (i, j) -> grid(i)(j)).toMap
    gridCoordMap
  }
